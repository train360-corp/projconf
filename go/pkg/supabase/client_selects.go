...
import (
	...
	"github.com/train360-corp/projconf/go/pkg/supabase/database"
	"io"
	"net/http"
	"strings"
)
...
func (c *Client) GetEnvironment(id *uuid.UUID) (*database.PublicEnvironmentsSelect, error) {
	endpoint := fmt.Sprintf("/environments?id=eq.%s", id.String())
...
func (c *Client) GetSecrets(environmentId string) (api.Secrets, error) {
	...
	endpoint := "/secrets?select=id%2Cvalue%2Cvariable%3Avariable_id(id%2Ckey)%2Cenvironment%3Aenvironment_id(id%2Cdisplay%2Cproject%3Aproject_id(id%2Cdisplay))"
	if environmentId != "" {
